...
async def startup():
    """Create DB tables at application startup.

    Wrap in try/except so the app can still start when DB credentials are
    invalid (useful during development). The error is logged so the user
    can fix the `DATABASE_URL` or fall back to SQLite.
    """
    try:
        Base.metadata.create_all(bind=engine)
        logging.info("Database tables created (or already exist).")
    except Exception as e:
        # Log a friendly message rather than crashing on import/startup.
        logging.warning(
            "Failed to create database tables on startup: %s.\n"
            "Check DATABASE_URL credentials or use a local SQLite fallback.",
            e,
        )
...
async def upload_receipt(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    try:
        ...
        log = HistoryStatus(receipt_id=new_receipt.id, previous_status="N/A", new_status="In Process", commentary="Uploaded via API")
        db.add(log)
        db.commit()
...
# webhook system
@app.get("/api/approve/{receipt_id}", response_class=HTMLResponse)
async def approve_receipt(receipt_id: int, db: Session = Depends(get_db)):
    ...
    # log history
    log = HistoryStatus(receipt_id=receipt_id, previous_status=previous_status, new_status="approved", commentary="Approved via Email")
    db.add(log)
    db.commit()
...
# webhook system
@app.get("/reject_form/{receipt_id}", response_class=HTMLResponse)
async def reject_form(receipt_id: int):
    return f"""
    <html>
        <body style="font-family: Arial; text-align: center; padding-top: 50px;">
            <h2 style="color: #d9534f;">Reject Receipt #{receipt_id}</h2>
            <form action="/api/reject/{receipt_id}" method="post" style="max-width: 400px; margin: 0 auto;">
...
@app.post("/api/reject/{receipt_id}", response_class=HTMLResponse)
async def process_rejection(receipt_id: int, reason: str = Form(...), db: Session = Depends(get_db)):
    receipt = db.query(Receipt).filter(Receipt.id == receipt_id).first()
    if not receipt:
        return HTMLResponse(content="<h1>Error: Receipt not found</h1>", status_code=404)
    ...
    #log history
    log = HistoryStatus(receipt_id=receipt_id, previous_status=previous_status, new_status="rejected", commentary=reason)
    db.add(log)
    db.commit()
