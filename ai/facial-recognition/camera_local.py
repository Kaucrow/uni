...
def load_db_config(filename_env_var='DB_CONFIG_FILE', section='postgresql'):
    """Reads database configuration parameters from a file path provided via environment variable."""
    filename = os.environ.get(filename_env_var)
    if not filename or not os.path.exists(filename):
        print(f"[ERROR] No se encontró la variable de entorno '{filename_env_var}' o el archivo de configuración '{filename}'.")
        # Fallback for testing, but should be set by the Streamlit app
        return None 
    ...
    try:
        ...
        print(f"[ERROR] Error al leer el archivo de configuración '{filename}': {e}")
        return None
    ...
    if parser.has_section(section):
        ...
        print(f"[ERROR] Sección '{section}' no encontrada en el archivo '{filename}'.")
        return None
    return db
...
def get_db_connection():
    ...
    if not config:
        print("[ERROR] No se pudo obtener la configuración de la base de datos.")
        sys.exit(1)
        
    try:
        conn = psycopg2.connect(**config)
        conn.autocommit = True # Important for web-app/script separation
        return conn
    except Exception as e:
        print(f"[FATAL] Error conectando a PostgreSQL: {e}")
        sys.exit(1)

# --- Database Initialization (Updated to PostgreSQL Syntax) ---
def init_database(conn):
    ...
    conn.commit() # Autocommit is set, but commit ensures tables exist.
    cur.close()
...
def load_person_embeddings(conn) -> dict[int, list[np.ndarray]]:
    cur = conn.cursor()
    # PostgreSQL uses BYTEA for binary data, which psycopg2 handles as bytes.
    cur.execute("SELECT id_persona, rostro FROM Rostro")
    embs: dict[int, list[np.ndarray]] = {}
    for pid, blob in cur.fetchall():
        try:
            # `blob` is a bytes object, so pickle.loads works directly
            emb = pickle.loads(blob)
            embs.setdefault(pid, []).append(normalize(emb))
        except Exception as e:
            # print(f"[WARN] Error loading embedding for pid {pid}: {e}")
            continue
    cur.close()
    return embs
...
def recognize(emb: np.ndarray, store: dict[int, list[np.ndarray]], thr: float) -> tuple[int | None, float]:
    ...
    for pid, lst in store.items():
        if not lst:
            continue
        try:
            # Scikit-learn's cosine_similarity is faster than pure numpy dot product for large lists
...
def register_mode(args):
    ...
    if args.person_id is not None:
        cur.execute("SELECT id FROM Persona WHERE id=%s", (args.person_id,))
        row = cur.fetchone()
        if not row:
    ...
    cur.close() # Close connection for now, reopened inside loop
    ...
    while True:
        ...
        if key == ord('c') and len(faces) > 0:
            ...
            # Save embedding
            cur = conn.cursor()
            # PostgreSQL: use BYTEA data type, pass the pickled bytes directly
            cur.execute("INSERT INTO Rostro(id_persona, rostro) VALUES(%s, %s)", (person_id, pickle.dumps(emb)))
            conn.commit()
            cur.close()
...
def detection_mode(args):
    ...
    conn_main.close() # Close main connection, inference thread will use its own
            ...
            try:
                ...
                if x0 is not None:
                    ...
                    if emb is not None:
                        ...
                        if pid is not None:
                            # Use %s placeholders for PostgreSQL query
                            cur_thr.execute("SELECT nombre, apellido FROM Persona WHERE id=%s", (pid,))
                    ...
                    if (emotion is not None) or (recog_conf is not None):
                        ...
                        if emotion is not None and emotion != last_logged_emotion:
                            ...
                            allow_log = True
                        if allow_log:
                            # Use %s placeholders and `NOW()` for PostgreSQL timestamp
                            cur_thr.execute(
                                "INSERT INTO Deteccion(id_persona, recog_confianza, emocion, emocion_confianza, timestamp) VALUES(%s, %s, %s, %s, NOW()) RETURNING id",
                                (pid, float(recog_conf) if recog_conf is not None else 0.0, emotion if emotion is not None else 'neutral', float(emo_conf) if emo_conf is not None else 0.0)
                            )
                            det_id = cur_thr.fetchone()[0]
                            # Persist full distribution if available
                            try:
                                if 'e_dist' in locals():
                                    for emo, confv in e_dist.items():
                                        cur_thr.execute(
                                            "INSERT INTO DeteccionEmocionDetalle(id_deteccion, emocion, confianza) VALUES(%s, %s, %s)",
...
def main():
    ...
    pr.add_argument('--nombre', type=str, help='Nombre (si no hay person')
    pr.add_argument('--apellido', type=str, help='Apellido')
    pr.add_argument('--email', type=str, help='Email')
    pr.add_argument('--duplicate-threshold', type=float, default=0.8, help='Umbral de duplicado para chequear')
    pr.add_argument('--force', action='store_true', help='Permite guardar incluso si detecta un duplicado')

    pd = sub.add_parser('detect', help='Deteccion en tiempo real y logging')
    pd.add_argument('--camera', type=int, default=0, help='Indice de camara (default 0)')
    pd.add_argument('--threshold', type=float, default=0.6, help='Umbral de similitud coseno para reconocimiento')
    pd.add_argument('--infer-interval-ms', type=int, default=500, help='Intervalo de inferencia de reconocimiento (ms)')
    pd.add_argument('--emotion-interval-ms', type=int, default=3000, help='Intervalo de inferencia de emocion (ms)')
    pd.add_argument('--scale', type=float, default=0.5, help='Factor de escala del rostro antes de embedding')
    pd.add_argument('--detect-scale', type=float, default=0.5, help='Factor de escala para deteccion de rostro (OpenCV/Haar)')
    pd.add_argument('--embed-model', type=str, default='Facenet', choices=['Facenet', 'ArcFace', 'VGG-Face'], help='Modelo para generar embeddings')
    pd.add_argument('--detector-backend', type=str, default='opencv', choices=['opencv', 'opencv-dnn', 'retinaface', 'mediapipe'], help='Backend para deteccion de rostro (caja verde)')
    pd.add_argument('--emotion-backend', type=str, default='opencv', choices=['opencv', 'retinaface', 'mediapipe', 'skip'], help='Backend para deteccion de rostro para emocion')
    pd.add_argument('--no-emotion', action='store_true', help='Deshabilitar analisis de emocion')
    pd.add_argument('--crop-padding', type=float, default=0.15, help='Padding (%) adicional en el recorte para emocion')
    pd.add_argument('--emotion-scale', type=float, default=1.2, help='Factor de upscale del rostro para emocion (mejora deteccion sutil)')
    pd.add_argument('--emo-smooth-frames', type=int, default=5, help='Frames para suavizado de emocion (EMA)')
    pd.add_argument('--emo-disgust-gain', type=float, default=1.6, help='Multiplica la prob. de disgusto antes de normalizar (calibracion)')
    pd.add_argument('--emo-balance', action='store_true', help='Balancear emociones por frecuencia inversa (EMA)')
    pd.add_argument('--emo-balance-strength', type=float, default=1.0, help='Fuerza del balanceo de emocion (beta)')
    pd.add_argument('--emo-balance-alpha', type=float, default=0.15, help='Alpha para EMA de balanceo de emocion')
    pd.add_argument('--min-log-interval-ms', type=int, default=5000, help='Intervalo minimo (ms) para loggear en BD')
    pd.add_argument('--frame-width', type=int, default=1280, help='Ancho de la resolucion de la camara')
    pd.add_argument('--frame-height', type=int, default=720, help='Alto de la resolucion de la camara')
    pd.add_argument('--force-mjpg', action='store_true', help='Forzar codec MJPG para reducir latencia')
    pd.add_argument('--target-fps', type=int, default=30, help='FPS objetivo de la camara')
    pd.add_argument('--box-smooth-alpha', type=float, default=0.5, help='Alpha para suavizado de caja de deteccion (EMA)')
