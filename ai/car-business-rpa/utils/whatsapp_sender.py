...
class WhatsAppSender:
    ...
    def send_graph(self, results: Dict[str, Any], destiny: str= None) -> bool:
        ...
        try:
            graph_message ="""
**Gráficos Generados:**
Se han generado varios gráficos para visualizar el análisis de ventas. 
...
"""
    ...
    def _format_summary(self, results: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # Se usa .iloc[0] para obtener la clave, ya que el índice puede ser un objeto Series/Index
            ...
            # Reestructurado para ser más directo y sin emojis
            parts = []
            parts.append("**Reporte de Análisis de Ventas**")
            parts.append(f"Clientes únicos: **{metrics['unique_clients']:,}**.")
            parts.append(f"Total de ventas (unidades): **{metrics['total_sales']:,}**.")
            parts.append(f"Ventas sin IGV: **${metrics['total_sales_without_igv']:,.2f}**.")
            parts.append(f"Ventas con IGV: **${metrics['total_sales_with_igv']:,.2f}**.")
            parts.append(f"IGV recaudado: **${metrics['total_igv_collected']:,.2f}**.")
            parts.append(f"Venta promedio: **${metrics['average_sales_without_igv']:,.2f}**.")
            
            parts.append("---") # Separador para mejor lectura
            
            parts.append(f"Modelo más vendido: **{top_models}**.")
            parts.append(f"Sede con más ventas: **{top_headquarter}**.")
            parts.append(f"Canal con más ventas: **{top_channel}**.")

            # ventas por sede en una lista
            hq_details = "\n".join([f"- {hq}: ${sales:,.2f}" for hq, sales in results['sales_by_headquarter'].items()])
            parts.append("\n**Ventas por Sede:**\n" + hq_details)
            ...
            parts.append("---")
            parts.append(f"Reporte generado: {self._get_today_date()}.")

            # Unir con saltos de línea para mejor formato en WhatsApp
            return "\n".join(parts)
        
        except Exception as e:
            logger.error(f"Error formateando resumen: {e}")
            return "Error formateando resumen de ventas."
            ...
            try:
                ...
                if imgbb_key and os.path.isdir(graphs_dir):
                    ...
                    if uploaded:
                        ...
                        message += f"\n\n**Gráficos en línea:** {urls_text}"
                else:
                    message += "\n\nLos gráficos del análisis se guardaron en la carpeta outputs/graphs."
            except Exception as e:
                logging.warning(f"No se pudieron subir los gráficos: {e}")
                message += "\n\nLos gráficos del análisis se guardaron en la carpeta outputs/graphs."
    ...
    def simulate_send_with_graph_urls(self, base_message: str) -> bool:
        ...
        try:
            ...
            if urls:
                message += "\n\n**Gráficos en línea (Simulado):** " + " ".join(urls)
            else:
                if graph_files:
                    # Fallback to local file paths if no URLs
                    message += "\n\n**Gráficos locales (Simulado):** " + " ".join(graph_files)
                else:
                    message += "\n\n**Aviso:** No se encontraron gráficos para adjuntar."
