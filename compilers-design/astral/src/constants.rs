...
impl TokenProto {
    pub fn to_token(&self, input: String) -> Token {
        match self {
            ...
            TokenProto::Comparison => Token::Comparison(input),
            TokenProto::LogicalAnd => Token::LogicalAnd,
            TokenProto::LogicalOr => Token::LogicalOr,
...
impl Token {
    pub fn proto(&self) -> TokenProto {
        match self {
            ...
            Token::Comparison(_) => TokenProto::Comparison,
            Token::LogicalAnd => TokenProto::LogicalAnd,
            Token::LogicalOr => TokenProto::LogicalOr,
...
pub const LEX_AHOCOR_DICT: Lazy<Vec<(TokenProto, Vec<&'static str>)>> = Lazy::new(|| vec![
    ...
    (TokenProto::Operator, vec!["<>", "<=", ">="]),
    (TokenProto::Operator, vec!["=", "<", ">", "&&", "||"]),
    /*(TokenProto::Comparison, vec!["<>", "<=", ">="]),
    (TokenProto::Comparison, vec!["=", "<", ">"]),
    (TokenProto::LogicalAnd, vec!["&&"]),
    (TokenProto::LogicalOr, vec!["||"]),*/
