...
namespace StudentFn {
    Student from_row(string row, int csv_pos) {
        ...
        CircularLinkedList<string> fields = CircularLinkedListFn::from_row(row);
        int field_num = 0;
        try {
            student.ci = stoi(fields[0]->data);
            student.first_name = fields[1]->data;
            student.last_name = fields[2]->data;
            student.email = fields[3]->data;
            switch(fields[4]->data[0]) {
                case 'M': student.gender = Gender::Male; break;
                case 'F': student.gender = Gender::Female; break;
                default: throw(nullptr);
            }
            student.av_grade = stof(fields[5]->data);
            student.school_year = stoi(fields[6]->data);
            student.section = fields[7]->data[0];
    ...
    CircularLinkedList<Student> list_from_csv(const char* csv_path) {
        CircularLinkedList<Student> linked_list;
    ...
    void obliterate_student(CircularLinkedList<Student> &list, const size_t idx, const char* csv_path) {
